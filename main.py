...
def pede_opcao():
    while True:
        opcao = input("Sua resposta (A/B/C/D/pula/ajuda): ").strip().lower()
...
def joga():
    ...
    for i, erro in enumerate(erros):
        if erro != {}:
            print(f"Erro na questão {i+1}: {erro}")
            return
    ...
    while True:
        if indice_premio == len(PONTOS):
            print(f"\nParabéns, {nome}! Você chegou a 1000000!")
            break
        ...
        if len(questoes_sorteadas) == len(base[nivel]):
            print("Não há mais questões inéditas nesse nível.")
            continue
        ...
        while True:
            print()
            print(questao_para_texto(questao, len(questoes_sorteadas)))
            print(f"\nPrêmio atual: {premio_atual}")
            print(f"Pulos restantes: {pulos_restantes}")
            print(f"Ajudas restantes: {ajudas_restantes}")
            ...
            if opcao == "ajuda":
                ...
                if ajuda_usada_nesta_questao:
                    print("Você já usou ajuda nesta questão.")
                    continue
            ...
            if opcao.upper() == questao["correta"]:
                ...
                print(f"Você الآن tem {premio_atual} pontos.")

                if indice_premio == len(PONTOS):
                    print(f"\nParabéns, {nome}! Você ganhou o prêmio máximo!")
                    return
                ...
                if continuar == "n":
                    print(f"Você saiu com {premio_atual} pontos.")
                    return
                break

            else:
                print("Resposta errada.")
                print("Você saiu sem prêmio.")
                return
